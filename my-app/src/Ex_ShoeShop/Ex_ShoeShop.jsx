...
const Ex_ShoeShop = () => {
  ...
  const addShoeCart = (shoe) => {
    ...
    }); //underfined== false nếu không tìm thấy thì ! == true
    let newArrCart = [...arrCart];
    console.log(check);
    if (check == -1) {
      // đây là trường hợp khi chưa có sản phẩm trong giỏ hàng
  ...
  const updateTotalShoeCart = (type, shoeId) => {
    // yêu cầu: thực hiện kiểm tra Transition, nếu lúc trừ total đi mà total đang là 1 thì sẽ xoá phần tử đó đi luôn
